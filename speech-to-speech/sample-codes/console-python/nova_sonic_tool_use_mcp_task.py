...
SQS_QUEUE_URL = "https://sqs.us-east-1.amazonaws.com/194722430130/strands-loom"
...
class BedrockStreamManager:
    ...
    async def initialize_stream(self):
        ...
        try:
            ...
            "If you are asked to do something that is beyond you capabilities, you create a task. The task will have access to those capabilities for you."
...
async def main(debug=False, disable_echo_cancellation=False, mcp_configs=None):
    ...
    async with MCPManager(mcp_configs or []) as mcp_manager:
        ...
        try:
            threading.Thread(target=sqs_polling, args=(stream_manager, sqs_queue, asyncio.get_event_loop(), stop_event)).start()
...
def sqs_polling(stream_manager: BedrockStreamManager, sqs_queue: Any, loop: asyncio.AbstractEventLoop, stop_event: threading.Event):
    while not stop_event.is_set():
        messages = sqs_queue.receive_messages(MaxNumberOfMessages=10, WaitTimeSeconds=20)
        for message in messages:
            try:
                print(f"Processing message: {message.body}")
                future = asyncio.run_coroutine_threadsafe(process_sqs_message(stream_manager, message.body), loop)
            except Exception as e:
                print(f"Error processing message: {e}")
                print(f"Skipping message: {message.body}")
                message.delete()
                continue
...
async def process_sqs_message(stream_manager: BedrockStreamManager, message_body: str):
    ...
    try:
        ...
        response = polly_client.synthesize_speech(
            ...
            SampleRate='16000'  # Match the input sample rate
        )
        ...
        chunk_size = 1024  # Match the CHUNK_SIZE used in the rest of the code
        for i in range(0, len(audio_stream), chunk_size):
            chunk = audio_stream[i:i + chunk_size]
            stream_manager.add_audio_chunk(chunk)
        ...
        print(f"Error processing message with Polly: {e}")
        if DEBUG:
            import traceback
            traceback.print_exc()
